...
def main():
    ...
    encodings = ["auto-detect", "utf-8", "latin1", "ISO-8859-1", "cp1252", "ascii"]
    encoding = st.sidebar.selectbox("File encoding:", encodings, index=0)
    ...
    if uploaded_file is not None:
        try:
            ...
            if encoding == "auto-detect":
                ...
                st.sidebar.info(f"Detected encoding: {detected_encoding}")
            ...
            st.sidebar.info("Try selecting a different encoding from the dropdown above")
                ...
                if not groq_api_key:
                    ...
                    try:
                        ...
                        if isinstance(response, dict) and 'output' in response:
                            clean_response = response['output']
                        else:
                        ...
                        if clean_response:
                            ...
                            *This analysis is based on the complete dataset containing {len(df)} rows and {len(df.columns)} columns.*
                            """
                            st.success("Query Result:")
            ...
            if operation_type == "Column Analysis":
                ...
                if viz_type == "Histogram":
                    ...
                    date_cols = [col for col in df.columns if df[col].dtype == 'datetime64[ns]' 
                        or (df[col].dtype == 'object' and pd.to_datetime(df[col], format='%Y-%m-%d', errors='coerce').notna().all())]
                    ...
                    if len(num_cols) >= 2:
                        ...
                        color_scheme = st.selectbox(
                            "Select color scheme:",
                            ["Dark Blue-Red", "Viridis (Dark)", "Plasma (Dark)", "Magma (Dark)", "Blues (Dark)"],
                            index=0
                        )
                        ...
                        if st.button("Generate Correlation Heatmap"):
                            ...
                            if color_scheme == "Dark Blue-Red":
                                cmap = sns.diverging_palette(220, 20, as_cmap=True)  
                            elif color_scheme == "Viridis (Dark)":
                                cmap = "viridis"
                            elif color_scheme == "Plasma (Dark)":
                                cmap = "plasma"
                            elif color_scheme == "Magma (Dark)":
                                cmap = "magma"
                            else:  
                                cmap = "Blues_r"  
                            ...
                            sns.heatmap(
                                ...
                                annot=True,
                                fmt=".2f",
                                annot_kws={"color": "white" if color_scheme != "Blues (Dark)" else "black"}
                            )
